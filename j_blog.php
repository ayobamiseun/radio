<?php 
	...
	if(!empty($_REQUEST['uid'])){
		...
		$sql = mysqli_query($con, "select * from news order by odr asc, id desc");
	}
	else{
		if(empty($_REQUEST['limit'])){
			$sql = mysqli_query($con, "select * from news where uid = '$_REQUEST[uid]'");
		}
		else{
			$limit = $_REQUEST['limit'];
			$sql = mysqli_query($con, "select * from news where uid !='$_REQUEST[uid2]' order by odr asc, id desc limit $_REQUEST[limit]");
		}
