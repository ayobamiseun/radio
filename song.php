<?php 
							...
							$uid = $_REQUEST['id'];
							$sqlm = mysqli_query($con,"select * from new_release where delStatus != 'deleted' AND uid = '$uid' order by id desc limit 1");
							...
							$m_details = $rowm['track_desc'];
							$m_artist= $rowm['artist_name'];
							...
							$upd = mysqli_query($con,"update new_release set view_count = '$vv' where uid = '$uid'");
...
<html lang="en">
<head>
<title><?php echo $m_title; ?> | KingsRadio</title>
<meta charset="utf-8">
									...
									<div class="show_comments">
										<a href="#">
											<div class="d-flex flex-row align-items-center justify-content-start">
												...
												<div class="show_comments_count ccoment">88 Comments</div>
											</div>
										</a>	
									</div>
								</div>
            ...
            $.getJSON('j_comments.php',{uid:uid}, function(data) {
             ...
             //alert(data.Feed.length);
              if(data.Feed.length > 1){
              	$('.ccoment').text(data.Feed.length+" comments");
              }
              else{
              	$('.ccoment').text(data.Feed.length+" comment");
              }
